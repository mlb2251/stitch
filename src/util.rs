...
// Print some info about a Vec of inventions.
pub fn inventions_info(inventions: &Vec<InventionExpr>) {
    let max_arity = inventions.iter().map(|p| p.arity).max().unwrap();
    println!("Inventions:");
    println!("\t num: {}", inventions.len());
    println!("\t max arity: {}", max_arity);
}

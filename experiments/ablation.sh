...
#for WL in "castle" "city" "house" ; do
    ...
    # force-multise = redundant-args in the paper
    # useless-abstract = argument-capture
    for OPTIM in "--no-opt-upper-bound" "--no-opt-force-multiuse" "--no-opt-useless-abstract" ; do
        echo "Running with OPTIM=$OPTIM"
        $STITCH_DIR/target/release/compress $WL_PATH $OPTIM --max-arity=3 --iterations=1 --no-mismatch-check --out=$OUT_DIR/raw/${WL}${OPTIM}.json > $OUT_DIR/stderr/${WL}${OPTIM}.stderr 2>&1 || true #&
    done
done
#wait $(jobs -rp)

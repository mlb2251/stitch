...
for BENCH_PATH in $BENCH_DIR/bench*.json; do
    ...
    /usr/bin/time -v $STITCH_DIR/target/release/compress $BENCH_PATH --max-arity=3 --threads=8 --iterations=$ITERATIONS --fmt=dreamcoder --out=$OUT_DIR/raw/$BENCH.json 2>&1 | tee $OUT_DIR/stderr/$BENCH.stderr
done
